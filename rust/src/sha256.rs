//! FIPS 180-4 SHA-256, so duplicate-group hashes in reports match the
//! identifiers TwinTidy's Go engine publishes. Implemented here because the
//! Rust standard library ships no cryptographic hash.
...
impl Sha256 {
    ...
    pub fn update(&mut self, mut data: &[u8]) {
        self.bit_length = self.bit_length.wrapping_add((data.len() as u64) * 8);
        while !data.is_empty() {
            let space = 64 - self.buffered;
    ...
    pub fn finish(mut self) -> [u8; 32] {
        ...
        // Append the length directly; update() would corrupt the counter.
...
mod tests {
    ...
    fn streaming_matches_single_shot() {
        ...
        let single = digest_of(&data);

        let mut hasher = Sha256::new();
        for chunk in data.chunks(37) {
            hasher.update(chunk);
        }
        assert_eq!(to_hex(&hasher.finish()), single);
    }
}
